...
import { FigmaExportDialog } from "@/components/tweakcn/figma-export-dialog";
import { SocialLink } from "@/components/tweakcn/social-link";
import { Button } from "@/components/ui/button";
import { Separator } from "@/components/ui/separator";
import { UserProfileDropdown } from "@/components/tweakcn/user-profile-dropdown";
      ...
      <div className="flex items-center justify-between gap-2 p-4">
        <div className="flex items-center gap-1">
          <Link href="/" className="flex items-center gap-2">
            <Logo className="size-6" title={t('logoTitle')} />
            <span className="hidden font-bold md:block">{t('logoText')}</span>
          </Link>
        </div>
